...
char	*ft_strjoin_with_equal(char *str1, char *str2)
{
	size_t	len1;
	size_t	len2;
	char	*result;
	...
	if (!result)
		exit (1);
	ft_lstadd_back22(&g_gg.lst_clct, ft_lstnew22(result));
...
void	join_auxiliaire(t_list	*tmp, char **result)
{
	int	i;
	...
	{
		if (tmp->value)
			result[i] = ft_strjoin_with_equal(tmp->name, tmp->value);
		else if (!(tmp->value))
			tmp = tmp->next;
		if (tmp)
			tmp = tmp->next;
